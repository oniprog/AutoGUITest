================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : SampleGUITest プロジェクトの概要
===============================================================================

この SampleGUITest アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、SampleGUITest アプリケーションを構成する各ファイルの内容の概要が含まれています。

SampleGUITest.vcxproj
...
SampleGUITest.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CSampleGUITestApp アプリケーション クラスを宣言します。

SampleGUITest.cpp
    これはメインのアプリケーション ソース ファイルで、CSampleGUITestApp アプリケーション クラスが含まれています。
...
res\SampleGUITest.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル SampleGUITest.rc にインクルードされます。

res\SampleGUITest.rc2
...
res\Toolbar.bmp
    ...
    初期ツール バーと初期ステータス バーは、CMainFrame クラスで生成されます。ツール バーのボタンを追加するには、リソース エディターを使用してこのツール バーのビットマップを編集し、SampleGUITest.rc 内の IDR_MAINFRAME TOOLBAR 配列を更新します。
/////////////////////////////////////////////////////////////////////////////

アプリケーション ウィザードは 1 つのドキュメントの種類と 1 つのビューを作成します。

SampleGUITestDoc.h, SampleGUITestDoc.cpp - ドキュメント
    これらのファイルには、CSampleGUITestDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CSampleGUITestDoc::Serialize を使用)。
    ドキュメントには次の文字列が含まれます。
        ファイル拡張子:      tst
        ファイルの種類の ID:        SampleGUITest.Document
        メイン フレームのキャプション:  SampleGUITest
        ドキュメントの種類の名前:       SampleGUITest
        フィルター名:         SampleGUITest Files (*.tst)
        ファイルの新しい短い名前: SampleGUITest
        ファイルの種類の長い名前: SampleGUITest.Document

SampleGUITestView.h, SampleGUITestView.cpp - ドキュメントのビュー
    これらのファイルには、CSampleGUITestView クラスが含まれます。
    CSampleGUITestView オブジェクトは CSampleGUITestDoc オブジェクトを表示するために使用されます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、SampleGUITest.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

SampleGUITest.manifest
